...
class BagEnv():
    ...
    def add_cylinder_base(self, bag_id=None):
        ...
        #beads = []
        #bead_positions_l = []
        part_shape = pb.createCollisionShape(pb.GEOM_CYLINDER, radius=radius*3, height=0.025)
        #part_visual = pb.createVisualShape(pb.GEOM_SPHERE, radius=radius*1.5)
        ...
        bag_vidx = min_idx_x
        bead_position = np.float32(verts_l[bag_vidx]) + np.array([0.025,0,0])
        part_id = pb.createMultiBody(0.01, part_shape, -1,
                basePosition=bead_position, baseOrientation= pb.getQuaternionFromEuler([0,1.57,0]))
        pb.changeVisualShape(part_id, -1, rgbaColor=color)
...
class ToteBag():
    ...
    def change_texture(self, deform_id):
        texture_id = pb.loadTexture('/home/clearlab/dedo/dedo/data/textures/deform/pb_white_knit.jpg')
        pb.changeVisualShape(deform_id, -1, rgbaColor=[1,1,1,1], textureUniqueId=texture_id)


class build_bag_base():

    def __init__(self):
        self.cartID = self.build_base()
        self.ee = 'suction'
        self.primitive = 'pick_place'
        self.max_steps = 11
        self._IDs = {}
    ...
    def build_base(self):
        cartID = pb.loadURDF("/home/clearlab/pybullet_robot/demos/pybullet_robot/robots/bullet_panda/models/cartpole.urdf",
                        [0.0, 2.0, 0.0],
        ...
        constraint_id = pb.createConstraint(cartID, 1, cartID, 3, jointType=pb.JOINT_GEAR, jointAxis=[0,1,0],parentFramePosition=[0,0,0],childFramePosition=[0,0,0])
        pb.changeConstraint(constraint_id, gearRatio=-1, maxForce=10000,erp=0.2)
        constraint_id2 = pb.createConstraint(cartID, 0, cartID, 2, jointType=pb.JOINT_GEAR, jointAxis=[0,1,0],parentFramePosition=[0,0,0],childFramePosition=[0,0,0])
        pb.changeConstraint(constraint_id2, gearRatio=-1, maxForce=10000,erp=0.2)
    ...
    def merge_base_with_bag(self, cart_id, bag_id):
        color = [078.0 / 255.0, 121.0 / 255.0, 167.0 / 255.0] + [1]
        # All tasks OTHER than bag-color-goal use self.bag_id. So if we are not
        # passing it in as an argument, we better have defined self.bag_id to use.
        if bag_id is None:
        ...
        '''
        #aa = np.array([13,  24,  35,  46,  57,  68,  79,  90, 101, 112, 123, 134, 145, 157, 168, 179, 190, 201, 212, 223, 234, 245, 256, 267, 278, 289,300, 311, 322, 333, 344, 350])
        aa = np.array([  5,  13,  14,  15,  16,  24,  25,  26,  27,  35,  36,  37,  38,
        46,  47,  48,  49,  57,  58,  59,  60,  68,  69,  70,  71,  79,
        80,  81,  82,  90,  91,  92,  93, 101, 102, 103, 104, 112, 113,
       114, 115, 123, 124, 125, 126, 134, 135, 136, 137, 145, 146, 147,
       148, 149, 157, 158, 159, 160, 168, 169, 170, 171, 179, 180, 181,
       182, 190, 191, 192, 193, 201, 202, 203, 204, 212, 213, 214, 215,
       223, 224, 225, 226, 234, 235, 236, 237, 245, 246, 247, 248, 256,
       257, 258, 259, 267, 268, 269, 270, 278, 279, 280, 281, 289, 290,
       291, 292, 300, 301, 302, 303, 311, 312, 313, 314, 322, 323, 324,
       325, 333, 334, 335, 336, 344, 345, 346, 347, 350, 351, 352])
        # Create constraint between a bead and certain bag vertices.
        _ = pb.createSoftBodyAnchor(
                softBodyBodyUniqueId=bag_id,
                nodeIndex=min_idx_x, 
        ...
        ####################################
        # Create soft anchors for the pillars
        ####################################
        
        bb = np.array([205, 211, 212, 213, 216, 217, 218, 220, 221, 222, 223, 224, 227,
       233, 234, 235])

        cc = np.array([28, 34, 35, 36, 39, 40, 41, 43, 44, 45, 46, 47, 50, 56, 57, 58])

        dd = np.array([116, 122, 123, 124, 127, 128, 129, 131, 132, 133, 134, 135, 138,
       144, 145, 146])

        ee = np.array([293, 299, 300, 301, 304, 305, 306, 308, 309, 310, 311, 312, 315,
       321, 322, 323])
        _link_name_to_index = {pb.getBodyInfo(cart_id)[0].decode('UTF-8'):-1,}
        for _id in range(pb.getNumJoints(cart_id)):
            _name = pb.getJointInfo(cart_id, _id)[12].decode('UTF-8')
            _link_name_to_index[_name] = _id
        print(_link_name_to_index)


        link_0 = pb.getLinkState(cart_id, 0)
        link_1 = pb.getLinkState(cart_id, 1)
        link_2 = pb.getLinkState(cart_id, 2)
        link_3 = pb.getLinkState(cart_id, 3)
        link_4 = pb.getLinkState(cart_id, 4)


        _, verts_l = pb.getMeshData(bag_id, -1, flags=pb.MESH_DATA_SIMULATION_MESH)
        verts_l = np.array(verts_l)
...
if __name__ == "__main__":
    ...
    cart_base = build_bag_base()
    cart_id = cart_base.return_cartID()
    rotate_base = pb.getQuaternionFromEuler([0, 3.14, 0])
    pb.resetBasePositionAndOrientation(cart_id, [0.5, 0.5, 0.9], rotate_base)
    bag2_id = cart_base.add_bag([0.5, 0.5, 0.9], [0, 0, -0.707, 0.707])
    rotate_bag2 = pb.getQuaternionFromEuler([-1.57, 0, 0])
    pb.resetBasePositionAndOrientation(bag2_id, [0.5, 0.5, 0.9], rotate_bag2)
    #cart_base.merge_base_with_bag(cart_id, bag2_id, rotate_base)
    ...
    #pb.changeDynamics(cartID, -1, linearDamping=0, angularDamping=0)
    #pb.changeDynamics(cartID, 0, linearDamping=0, angularDamping=0)
    #pb.changeDynamics(cartID, 1, linearDamping=0, angularDamping=0)
    ...
    #################################
    #Add bag from transporter network
    #################################
    bag_class = BagEnv()
    bag_id = bag_class.add_bag([0.3, 0.3, 0.9], [0, 0, -0.707, 0.707])
    counttt = 0
    ...
    counttttt = 0
    while True:
        pb.applyExternalForce(bag_id, 1, [5,0,20000],[0,0,0],pb.WORLD_FRAME)
        print(counttttt)
        counttttt+=1
        pb.stepSimulation()
        if counttttt == 500:
            cart_base.merge_base_with_bag(cart_id, bag2_id)
